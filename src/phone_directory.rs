...
fn filter_wbs(lines: String) -> String {
    let mut last_is_wb = false;
    lines
        .chars()
        .filter(|c| {
            if *c == ' ' && last_is_wb {
...
fn phone(dir: &str, num: &str) -> String {
    ...
    let line: Vec<&str> = dir.lines().filter(|l| l.contains(&_num)).collect();
    if line.is_empty() {
        format!("Error => Not found: {}", num)
    } else if line.len() > 1 {
        format!("Error => Too many people: {}", num)
    } else {
        lazy_static! {
            static ref RE: Regex = Regex::new(r"<(.*)>").unwrap();
        }
        let name: String = RE.captures(line[0]).map(|cap| cap[1].to_string()).unwrap();

        let string_line = line[0].to_string();
        let string_line = string_line.replace(num, "").replace(name.as_str(), "");
        let string_line = string_line
            .replace(";", "")
            .replace("+", "")
            .replace("$", "");
        let string_line = filter_wbs(string_line);

        lazy_static! {
            static ref RE2: Regex = Regex::new(r"((\w[\S]*)\s)*\w[\S]*").unwrap();
        }
        let address: String = RE2
            .captures(string_line.as_str())
            .map(|cap| cap[0].to_string())
            .unwrap();
        let address = address.replace("_", " ");
        format!("Phone => {}, Name => {}, Address => {}", num, name, address)
    }
}
